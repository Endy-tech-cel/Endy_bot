...
let handler = async (m, { text, usedPrefix, command }) => {
  ...
  try {
    const basePrompt = `Il tuo nome è EndBot e sei stato creato da Endy. Usi la lingua italiana. Sii coinvolgente, simpatico, diretto, curioso e rispondi sempre in modo chiaro e utile. Se puoi, aggiungi un tocco di ironia e stimola la conversazione. Le tue risposte devono essere brevi e concise.`
    const chatId = m.chat
...
async function getGeminiResponse(apiKey, prompt, question, model) {
  ...
  const response = await fetch(
    `https://generativelanguage.googleapis.com/v1/models/${model}:generateContent?key=AIzaSyDG70wvOfP2e-qEX78wT9RGZ4kAGe0Q2r0`,
    {
      method: 'POST',
      headers: {
